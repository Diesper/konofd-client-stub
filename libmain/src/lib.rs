    ...
    PATCH_METHOD_HOOK => {
      ...
      fn get_il2cpp_data_dir() -> String {
        ...
        let data_dir = unsafe {
          ...
          let data_dir =
            std::slice::from_raw_parts(data_dir.ptr as *const u8, data_dir.len as usize);
          debug!("DataDir: {:?}", data_dir);
      ...
      unsafe fn LoadMetadataFile_patched(file_name: *const c_char) -> u64 {
        ...
        let static_url = "https://axel.assasans.dev/static/";
...
"#;
        ...
        info!(
          "loaded metadata file, size: {}, ptr: 0x{:x}",
          leaked_bytes.len(),
          ptr
        );
